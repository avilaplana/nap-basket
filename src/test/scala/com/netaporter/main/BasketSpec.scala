...
import com.netaporter.domain.{Basket, Product, ProductNotFound, ProductNotValid}
...
class BasketSpec extends WordSpec with Matchers {
  ...
  "add" should {
    ...
    "return a immutable Basket with Product1 and quantity 1 when Product1 is added to empty basket" in {
      implicit val exist: String => Option[Product] = id => Some(ProductBuilder(id))
    ...
    "return ProductNotValid error when Product not valid is added to a Basket" in {
      implicit val exist: String => Option[Product] = id => None
...
object ProductBuilder {
  ...
  def apply(id: String, price: Double) = Product(id, "some name", price)
}
